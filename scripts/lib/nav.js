...
function renderFooter(nav, siteUrl) {
  ...
  const links = (f.links || []).map(l => `<li>${a(l, siteUrl)}${esc(l.label)}</a></li>`).join('');
  const sub = f.subscribe;
  return `<footer class="site-footer">
  <div class="fwrap">
    <div class="ftop">
      <div class="fbrand">
        <div class="fident"><img src="${esc(f.mark.src)}" alt="${esc(f.mark.alt)}" width="${f.mark.width || 58}" height="${f.mark.height || 58}" loading="lazy"><span class="fname">${esc(f.name)}</span></div>
        <p class="ftag">${esc(f.tagline)}</p>
      </div>
      <div class="fsub">
        <form class="fform" action="${esc(sub.action)}" method="post" data-subscribe>
          <label class="sr" for="footer-email">Email address</label>
          <input id="footer-email" type="email" name="email" placeholder="${esc(sub.placeholder)}" required autocomplete="email">
          <button type="submit">${esc(sub.label)}</button>
        </form>
        <ul class="fsocial">${social}</ul>
      </div>
    </div>
    <hr class="frule">
    <ul class="flinks">${links}</ul>
...
(function(){
  ...
  f.addEventListener('submit', async function(e){
    ...
    try{
      ...
      if(out.ok){ btn.textContent='SUBSCRIBED'; input.disabled=true; }
