...
3. **Netlify Blobs fails loudly on the deployed site** if it is unavailable (no silent fallback), so duplicates cannot slip through. In local `npm run dev` a JSON file under `.netlify/blobs-local/` stands in.
4. **First npm dependency:** `@netlify/blobs`. Netlify installs it at build; run `npm install` locally once.
5. **Race window:** the Blobs store is eventually consistent on this runtime (strong reads are not available in Lambda-style functions), so two people submitting the same number within a few seconds of each other could both pass the check. Acceptable for an opt-in form; the rebuild script reports any duplicates it finds.
